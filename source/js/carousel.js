(function () {
  class Carousel {
    constructor(obj) {
      ...
      this.list = this.container.querySelector(
        `.${this.name}__${obj['listName']}`
      );
      this.items = this.container.querySelectorAll(
        `.${this.name}__${obj['itemsName']}`
      );
      ...
      if (obj['hasRadioBtns']) {
        ...
        this.refreshRadioBtns = () => {
          this.radioBtns = document.querySelectorAll(
            `.${this.name}__radio-btn`
          );
        };
        if (this['items'].length > this.itemsVisible()) {
          this.adjustRadioBtns(this);
        }
      }
      if (obj['hasNextPreviousBtns']) {
        this.btnPrevious = document.querySelector(
          `.${this.name}__btn--previous`
        );
    ...
    addEventListeners(context) {
      ...
      this['container'].addEventListener('mousedown', function (event) {
        ...
        context.longPress = setTimeout(function () {
      ...
      window.addEventListener('resize', function () {
        if (
          context['currentItem'] + context.itemsVisible() >=
          context['items'].length
        ) {
          context['currentItem'] =
            context['items'].length - context.itemsVisible();
        }
        if (
          typeof context['radioBtns'] !== 'undefined' &&
          context['items'].length > context.itemsVisible()
        ) {
          context.adjustRadioBtns(context);
        }
        context.adjust();
      });
      ...
      if (this['btnPrevious']) {
        this.addEventListenersToNextPreviousBtns(this);
      }
      if (this['modalCarouselProperties']) {
        this.addEventListenersToExpandCarousel(this);
      }
    }
    ...
    adjust() {
      ...
      if (this['resizeToFill']) {
        this['list'].style.width =
          (itemWidth + marginBetweenItems) * this['items'].length -
          marginBetweenItems +
          'px';
      }
      this['list'].style.transform =
        'translateX(' + this['currentItem'] * this['stepWidth'] * -1 + 'px)';
      if (this['radioBtns']) {
        this.whichRadioBtnChecked();
      }
    }
    adjustRadioBtns(context) {
      let numberOfBtns = Math.ceil(this['items'].length / this.itemsVisible());
      if (numberOfBtns === this['radioBtns'].length) {
        return;
      } else {
        this['radioBtnsContainer'].innerHTML = '';
      }
      ...
      for (let i = 0; i < numberOfBtns; i++) {
        ...
        newLabel.innerHTML =
          '<span class="visually-hidden">Button №' + (i + 1) + '<span>';
      ...
      for (let i = 0; i < labelsArr.length; i++) {
        this['radioBtns'][i].oninput = () => {
          let position;
          i !== labelsArr.length - 1
            ? (position = i * context.itemsVisible())
            : (position = context['items'].length - context.itemsVisible());
    ...
    whichRadioBtnChecked() {
      ...
      for (let i = 0; i < this['radioBtns'].length; i++) {
        if (
          this['currentItem'] <= itemsVisible * (i + 1) - 1 &&
          this['currentItem'] > itemsVisible * i - 1
        ) {
          this['radioBtns'][i].checked = true;
        }
      }
      if (
        this['currentItem'] + 1 >=
        itemsVisible * Math.floor(this['items'].length / itemsVisible)
      ) {
        this['radioBtns'][this['radioBtns'].length - 1].checked = true;
      }
    }
    ...
    slideStoppedAtEdge(direction, context) {
      ...
      for (let item of this['items']) {
        item.classList.add('carousel__at-the-edge--' + direction);
      }
      setTimeout(() => {
        for (let item of context['items']) {
          item.classList.remove('carousel__at-the-edge--' + direction);
        }
      }, this['animationSpeed']);
    }
    slideBeyond(direction, context) {
      for (let item of this['items']) {
        item.classList.add('carousel__slide--' + direction);
      }
      if (direction === 'left') {
      ...
      setTimeout(() => {
        for (let item of context['items']) {
          item.classList.remove('carousel__slide--' + direction);
        }
      }, this['animationSpeed']);
      this.refreshItems();
      this['items'][this['currentItem']].classList.add(
        'carousel__current-item'
      );
      this.adjust();
    }
    slide(direction) {
      ...
      this['items'][this['currentItem']].classList.remove(
        'carousel__current-item'
      );
      if (direction === 'right') {
        this['currentItem']++;
      ...
      if (this['currentItem'] < 0) {
        this['stopAtEdge']
          ? this.slideStoppedAtEdge(direction, context)
          : this.slideBeyond(direction, context);
        return;
      } else if (
        this['currentItem'] >
        this['items'].length - this.itemsVisible()
      ) {
        this['stopAtEdge']
          ? this.slideStoppedAtEdge(direction, context)
          : this.slideBeyond(direction, context);
    ...
    showModalCarousel(event) {
      if (!this['childCarousel']) {
        this['childCarousel'] = new ModalCarousel(
          this['modalCarouselProperties'],
          this
        );
      }
  ...
  class ModalCarousel extends Carousel {
    constructor(obj, parentCarousel) {
      ...
      this.thumbnailsWrapper = document.querySelector(
        `.${this.name}__thumbnails-wrapper`
      );
      this.refreshThumbnails = () => {
        this.thumbnails = this.thumbnailsList.querySelectorAll(
          `.${this.name}__thumbnail`
        );
      };
    ...
    addEventListeners(context) {
      ...
      this['btnCollapse'].addEventListener('click', function () {
        context['thumbnailsWrapper'].classList.toggle(
          'modal-gallery__thumbnails-wrapper--hide'
        );
      });
      this['container'].addEventListener('dragstart', function (event) {
        event.preventDefault();
      ...
      this['container'].addEventListener('mousedown', function (event) {
        ...
        context['swipeInformation'] = eventInformation;
        context.longPress = setTimeout(function () {
          context.carouselCore['swipeInformation'] = null;
        }, 2500);
      });
      this['container'].addEventListener(
        'touchstart',
    ...
    copyImgsToCarousel(parentCarousel) {
      ...
      for (let i = 0; i < parentCarousel['items'].length; i++) {
        ...
        imgWrapper.style.marginRight =
          Math.ceil(document.body.clientWidth / 2) - elementWidth + 'px';
        ...
        img.src = galeryImg.src
          .replace('[.][a-zA-Z]{3,4}', '-uc.jpg')
          .replace('@2x', '');
    ...
    synchronizeModalCarousel() {
      for (let i = 0; i < this['items'].length; i++) {
        this['thumbnails'][i].classList.remove(
          'modal-gallery__thumbnail--selected'
        );
        if (
          this['items'][this['currentItem']].dataset.id ===
          this['thumbnails'][i].dataset.id
        ) {
          this['thumbnails'][i].classList.add(
            'modal-gallery__thumbnail--selected'
          );
        }
      }
      this['btnDownload'].href = this['items'][
        this['currentItem']
      ].querySelector('img').src;
      this['imgCounter'].textContent =
        +this['items'][this['currentItem']].dataset.id +
        1 +
        '/' +
        this['items'].length;
    ...
    chooseImgOnThumbnail(element) {
      if (element.classList.contains('modal-gallery__thumbnail--selected')) {
        return;
      }
